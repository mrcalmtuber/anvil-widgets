...
## Step 1 — Clone the project onto your Mac

Download or clone this repository, then open Terminal and navigate to the `artifacts/widget-hub` folder:

```bash
cd path/to/artifacts/widget-hub
```
...
## Step 3 — Build the web app for Capacitor

This creates a `dist/` folder using `./` as the base path (required — different from the Replit dev build):
...
## Step 4 — Add the iOS native project
...
## Step 5 — Sync web assets into iOS
...
## Step 10 — Generate a sideload .ipa (Ad Hoc)
...
## Step 11 — TestFlight (recommended before App Store)
...
## Step 12 — Submit to the App Store
...
When you make changes in Replit, export the code and on your Mac run:
```bash
npx vite build --config vite.capacitor.config.ts
npx cap sync ios
# Open Xcode, bump the version number, Archive, and upload
```
